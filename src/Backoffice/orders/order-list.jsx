...
const handleCloseModal = () => {
 ...
 // AET
  const filteredRows = rows.filter(row =>
    row.customer_id.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const deleteApi = async (id) => {
    ...
    axios.delete(`${BASE_URL}/api/orders/${id}`) 
      .then(() => {
        setRows(rows.filter((order) => order._id !== id)); // Corrected this line
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left" >
                      {/* <a href="/orderDetails">#{row.reford}</a> */}
                     #{row.reford} 
                      </TableCell>
                    <TableCell
                      component="th"
                    ...
                    <TableCell align="right">
                         <Label  color={(row.status === 'pending' && 'warning') ||(row.status === 'shipped' && 'secondary') || (row.status === 'delivered' && 'primary') || (row.status === 'cancelled' && 'error') || 'success'}>{row.status }</Label>
                    </TableCell>
                    
                    
                    <TableCell align="right">
                         {/* Format the created_at here */}
                         {new Date(row.created_at).toLocaleString()} {/* Example format */}
                   </TableCell>

                   <TableCell align="right">
                         {/* Format the updated_at here */}
                         {new Date(row.updated_at).toLocaleString()} {/* Example format */}
                   </TableCell>
            ...
            <Fade in={editModalOpen}>
                <Card sx={style}>
                    <AddOrderForm
                        ...
                        user={selectedUser}
                        closeEvent={() => setEditModalOpen(false)}
                        isEdit={isEdit} // Pass isEdit to AddOrderForm
